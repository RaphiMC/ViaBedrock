...
public class EntityTracker extends StoredObject {
    ...
    public Entity addEntity(final long uniqueId, final long runtimeId, final String type, final EntityTypes1_21_5 javaType) {
        final UUID javaUuid = UUID.randomUUID();
        if (javaType.isOrHasParent(EntityTypes1_21_5.ABSTRACT_HORSE)) {
        ...
        } else if (javaType.isOrHasParent(EntityTypes1_21_5.MOB)) {
            return this.addEntity(new MobEntity(this.user(), uniqueId, runtimeId, type, this.getNextJavaEntityId(), javaUuid, javaType));
        } else if (javaType.isOrHasParent(EntityTypes1_21_5.LIVING_ENTITY)) {
    ...
    public void spawnItemFrame(final BlockPosition position, final BlockState blockState) {
        ...
        final PacketWrapper spawnEntity = PacketWrapper.create(ClientboundPackets1_21_5.ADD_ENTITY, this.user());
        spawnEntity.write(Types.VAR_INT, javaId); // entity id
        spawnEntity.write(Types.UUID, UUID.randomUUID()); // uuid
        spawnEntity.write(Types.VAR_INT, blockState.identifier().equals("frame") ? EntityTypes1_21_5.ITEM_FRAME.getId() : EntityTypes1_21_5.GLOW_ITEM_FRAME.getId()); // type id
