...
public class FlowerPotBlockEntityRewriter implements BlockEntityRewriter.Rewriter {
    ...
    public BlockEntity toJava(UserConnection user, BedrockBlockEntity bedrockBlockEntity) {
        ...
        if (bedrockTag.get("PlantBlock") instanceof CompoundTag) {
            ...
            final int bedrockBlockState = blockStateRewriter.bedrockId(blockTag);
            if (bedrockBlockState == -1) {
                ViaBedrock.getPlatform().getLogger().log(Level.WARNING, "Missing block state: " + blockTag);
                return new BlockEntityWithBlockState(javaBlockEntity, DEFAULT_BLOCK_STATE);
            }
