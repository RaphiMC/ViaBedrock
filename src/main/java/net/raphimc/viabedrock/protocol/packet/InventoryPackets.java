...
public class InventoryPackets {
    ...
    public static void register(final BedrockProtocol protocol) {
        ...
        protocol.registerClientbound(ClientboundBedrockPackets.CONTAINER_CLOSE, ClientboundPackets1_21_6.CONTAINER_CLOSE, new PacketHandlers() {
            @Override
        ...
        protocol.registerClientbound(ClientboundBedrockPackets.INVENTORY_CONTENT, ClientboundPackets1_21_6.CONTAINER_SET_CONTENT, wrapper -> {
        ...
        protocol.registerClientbound(ClientboundBedrockPackets.INVENTORY_SLOT, ClientboundPackets1_21_6.CONTAINER_SET_SLOT, wrapper -> {
            final ItemRewriter itemRewriter = wrapper.user().get(ItemRewriter.class);
            final int containerId = wrapper.read(BedrockTypes.UNSIGNED_VAR_INT); // container id
            ...
            if (container != null && container.setItem(slot, item)) {
                if (container.type() == ContainerType.HUD && slot == 0) { // cursor item
                    wrapper.setPacketType(ClientboundPackets1_21_6.SET_CURSOR_ITEM);
                } else {
                ...
                wrapper.write(VersionedTypes.V1_21_6.item, container.getJavaItem(slot)); // item
            } else {
                wrapper.cancel();
            }
        });
        ...
        protocol.registerServerbound(ServerboundPackets1_21_6.SET_CREATIVE_MODE_SLOT, null, wrapper -> {
            wrapper.cancel();
            final short slot = wrapper.read(Types.SHORT); // slot
            final Item item = wrapper.read(VersionedTypes.V1_21_6.lengthPrefixedItem); // item
