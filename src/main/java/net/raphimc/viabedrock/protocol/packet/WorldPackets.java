...
public class WorldPackets {
    ...
    public static void register(final BedrockProtocol protocol) {
        ...
        protocol.registerClientbound(ClientboundBedrockPackets.SET_TIME, ClientboundPackets26_1.SET_TIME, wrapper -> {
            ...
            final long javaTime = bedrockTime >= 0 ? bedrockTime % 24000L : 24000 + (bedrockTime % 24000L);

            wrapper.write(Types.VAR_INT, 1); // One!
            wrapper.write(Types.VAR_INT, 0); // Overworld clock
            wrapper.write(Types.VAR_LONG, javaTime); // Total ticks
            wrapper.write(Types.FLOAT, 0F); // Partial tick
            wrapper.write(Types.FLOAT, wrapper.user().get(GameRulesStorage.class).getGameRule("doDayLightCycle") ? 1F : 0F); // Tick rate
        });
