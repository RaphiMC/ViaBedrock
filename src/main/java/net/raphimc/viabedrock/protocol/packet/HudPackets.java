...
public class HudPackets {
    ...
    public static void register(final BedrockProtocol protocol) {
        protocol.registerClientbound(ClientboundBedrockPackets.PLAYER_LIST, ClientboundPackets1_21_6.PLAYER_INFO_UPDATE, wrapper -> {
            ...
            switch (action) {
                case Add -> {
                    ...
                    if (!toRemoveUUIDs.isEmpty()) {
                        ...
                        final PacketWrapper playerInfoRemove = PacketWrapper.create(ClientboundPackets1_21_6.PLAYER_INFO_REMOVE, wrapper.user());
                ...
                case Remove -> {
                    wrapper.setPacketType(ClientboundPackets1_21_6.PLAYER_INFO_REMOVE);
        ...
        protocol.registerClientbound(ClientboundBedrockPackets.SET_TITLE, null, wrapper -> {
            ...
            try {
                ...
                switch (type) {
                    case Clear, Reset -> {
                        wrapper.setPacketType(ClientboundPackets1_21_6.CLEAR_TITLES);
                        wrapper.write(Types.BOOLEAN, type == SetTitlePacket_TitleType.Reset); // reset
                    }
                    case Title, TitleTextObject -> {
                        wrapper.setPacketType(ClientboundPackets1_21_6.SET_TITLE_TEXT);
                        wrapper.write(Types.TAG, TextUtil.stringToNbt(text)); // text
                    }
                    case Subtitle, SubtitleTextObject -> {
                        wrapper.setPacketType(ClientboundPackets1_21_6.SET_SUBTITLE_TEXT);
                        wrapper.write(Types.TAG, TextUtil.stringToNbt(text)); // text
                    }
                    case Actionbar, ActionbarTextObject -> {
                        wrapper.setPacketType(ClientboundPackets1_21_6.SET_ACTION_BAR_TEXT);
                        wrapper.write(Types.TAG, TextUtil.stringToNbt(text)); // text
                    }
                    case Times -> {
                        wrapper.setPacketType(ClientboundPackets1_21_6.SET_TITLES_ANIMATION);
        ...
        protocol.registerClientbound(ClientboundBedrockPackets.SET_DISPLAY_OBJECTIVE, ClientboundPackets1_21_6.SET_DISPLAY_OBJECTIVE, wrapper -> {
            final ScoreboardTracker scoreboardTracker = wrapper.user().get(ScoreboardTracker.class);
            ...
            if (!scoreboardTracker.hasObjective(objectiveName)) {
                ...
                final PacketWrapper scoreboardObjective = PacketWrapper.create(ClientboundPackets1_21_6.SET_OBJECTIVE, wrapper.user());
        ...
        protocol.registerClientbound(ClientboundBedrockPackets.REMOVE_OBJECTIVE, ClientboundPackets1_21_6.SET_OBJECTIVE, new PacketHandlers() {
            @Override
        ...
        protocol.registerClientbound(ClientboundBedrockPackets.BOSS_EVENT, ClientboundPackets1_21_6.BOSS_EVENT, wrapper -> {
