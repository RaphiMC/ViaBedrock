...
Set `-Dviabedrock.generator.output=/path/to/dumps` as a Java argument to change the output directory. Each client launch clears the previous registry and tag dumps. Wait for login to finish before copying those files. Review entity field names against ViaBedrock's entity enum because the name conversion is version specific.
